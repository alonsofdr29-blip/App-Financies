...
const CATEGORY_META = {
  Comida: { icon: "🍔", color: "#d97706" },
  Casa: { icon: "🏠", color: "#0f766e" },
  Transporte: { icon: "🚗", color: "#334155" },
  Ocio: { icon: "🎮", color: "#be123c" },
  Salud: { icon: "💊", color: "#0d9488" },
  Suscripciones: { icon: "📺", color: "#1e293b" },
  Sueldo: { icon: "💼", color: "#047857" },
  Extra: { icon: "💰", color: "#65a30d" },
  Ventas: { icon: "🛒", color: "#0891b2" },
  Regalo: { icon: "🎁", color: "#e11d48" },
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-900/60">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="w-28 shrink-0">
          <div className="mb-1 text-right text-[11px] font-bold text-slate-500 dark:text-slate-300">
            {b > 0 ? `${pct}%` : "—"}
          </div>
          <input
            value={String(budget ?? "")}
            ...
            placeholder="€"
            type="number"
            className="w-full rounded-xl border border-slate-200 bg-white px-2 py-2 text-sm font-semibold outline-none focus:ring-2 focus:ring-teal-200 dark:border-slate-700 dark:bg-slate-900/70 dark:text-white dark:focus:ring-teal-700/40"
          />
        </div>
      </div>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen text-slate-900 dark:text-slate-100">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(950px_520px_at_5%_0%,rgba(13,148,136,0.14),transparent_62%),radial-gradient(820px_560px_at_95%_8%,rgba(15,23,42,0.08),transparent_62%)] dark:bg-[radial-gradient(900px_620px_at_15%_0%,rgba(13,148,136,0.16),transparent_62%),radial-gradient(900px_540px_at_90%_5%,rgba(15,23,42,0.48),transparent_62%)]" />
      <div className="relative">
        <div className="mx-auto w-full px-4 py-4 lg:px-8 lg:py-4 lg:h-[calc(100vh-24px)]">
            {/* Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-3xl bg-gradient-to-br from-slate-900 to-teal-700 text-white shadow-[0_10px_24px_rgba(15,23,42,0.3)]">
            ...
            {/* Dashboard */}
            {/* (aquí sigue EXACTAMENTE tu UI como estaba) */}
                  ...
                  <SmallCard className="p-4">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <input
                        ...
                        className="rounded-2xl border px-3 py-2 text-sm font-semibold outline-none border-slate-200 bg-white text-slate-900 focus:ring-2 focus:ring-teal-200 dark:border-slate-700 dark:bg-slate-900/70 dark:text-white dark:focus:ring-teal-700/40"
                      />
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      ...
                      <SmallCard className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-50 dark:bg-rose-500/10">
                  ...
                  <SmallCard className="p-4">
                    ...
                    <div className="mt-3 grid gap-3">
                      ...
                      <Input
                        ...
                        type="number"
                        right={<span className="text-sm font-extrabold text-slate-500">€</span>}
                      />
                      ...
                      <div>
                          ...
                          {(SUGGESTED_CATEGORIES[kind] || []).map((c) => (
                            <button
                              ...
                              className={`rounded-full px-3 py-1 text-xs font-bold ring-1 transition ${
                                (category || "").trim() === c
                                  ? "bg-slate-900 text-white ring-slate-900 dark:bg-teal-500 dark:text-slate-950 dark:ring-teal-400"
                                  : "bg-white text-slate-700 ring-slate-200 hover:bg-slate-50 dark:bg-slate-900/70 dark:text-slate-200 dark:ring-slate-700"
                              }`}
                            >
                              {c}
                            </button>
                        ...
                        <div className="mt-2">
                          <input
                            ...
                            className="w-full rounded-2xl border px-3 py-3 text-sm outline-none border-slate-200 bg-white text-slate-900 focus:ring-2 focus:ring-teal-200 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100 dark:focus:ring-teal-700/40"
                          />
                        </div>
                      </div>
                ...
                <div className="flex flex-col gap-4 lg:h-full lg:overflow-hidden min-h-0">
                  ...
                  <SmallCard className="p-4 lg:flex-1 lg:flex lg:flex-col lg:overflow-hidden min-h-0">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="relative flex items-center gap-2 rounded-2xl border px-3 py-2 border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900/70">
                          <input
                            placeholder="Buscar…"
                            value={query}
                        ...
                        <select
                          ...
                          className="rounded-2xl border px-3 py-2 text-sm font-bold outline-none border-slate-200 bg-white text-slate-900 dark:border-slate-700 dark:bg-slate-900/70 dark:text-white"
                        >
                    ...
                    <div className="mt-3 space-y-2 lg:flex-1 lg:overflow-auto lg:pr-2 min-h-0">
                      <AnimatePresence>
                        {filteredItems.length === 0 ? (
                          <div className="rounded-2xl bg-slate-50 p-4 text-center text-sm text-slate-500 dark:bg-slate-900/60 dark:text-slate-300">
                            No hay movimientos
                          </div>
                        ) : (
                          filteredItems.map((it) => (
                            <motion.div
                              ...
                              exit={{ opacity: 0, y: 6 }}
                              className="flex items-center justify-between rounded-3xl border p-4 border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900/60 dark:shadow-[0_1px_0_rgba(255,255,255,0.06)]"
                            >
                              <div className="flex items-center gap-3">
                                <Badge
                              ...
                              <div className="flex items-center gap-3">
                                <div
                                  className={
                                    "font-extrabold " + (it.kind === "income" ? "text-emerald-700" : "text-rose-700")
                                  }
                                >
                                  {eur(it.amount)}
                                </div>
                                <button
                                  ...
                                  className="inline-flex items-center justify-center rounded-2xl p-2 bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800/80 dark:text-slate-200 dark:hover:bg-slate-700"
                                  type="button"
                                >
